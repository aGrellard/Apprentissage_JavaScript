...
let result = ["A", "C", "G", "T"].map(item=>split[item]).join(" ")
...
// sortie :
// false
if (anagrams[0].length == anagrams[1].length) {
    let string_1 = anagrams[0]
    let string_2 = anagrams[1]
    let string_1Sorted = string_1.split('').sort().join("");
    let string_2Sorted = string_2.split('').sort().join("");
    if (string_1Sorted === string_2Sorted) {
